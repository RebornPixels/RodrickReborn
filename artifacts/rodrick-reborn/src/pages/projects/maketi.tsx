      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.2, duration: 0.5 }}>
          ...
          <p className="text-lg text-gray-700 dark:text-gray-300 mb-6 leading-relaxed">
            An e-commerce marketplace tailored for Malawi, featuring local mobile money (Airtel API), vendor stores, product ratings, and localized logistics.
          </p>
          ...
          <div className="flex gap-4">
            ...
            <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href="https://maketi.vercel.app" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-5 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
        ...
        <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.4, duration: 0.5 }} className="relative">
          <div className={`relative rounded-2xl overflow-hidden shadow-xl ${isDark ? 'shadow-green-900/20' : 'shadow-gray-400/30'}`}>
            <img src={isDark ? '/images/maketi_dark.png' : '/images/maketi_light.png'} alt="Malawi Maketi Screenshot" className="w-full h-auto rounded-2xl border border-gray-200 dark:border-gray-700" />
          </div>
        </motion.div>
      </div>
    </motion.section>
